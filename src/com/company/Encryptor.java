...
public class Encryptor {
    ...
    public byte[] crypt(byte[] message, byte[] key, byte[] iv, String operation) {
        ...
        try {
            if (length == 8) {
                if (mode.equals("CBC")) {
                    ...
                    cipher = cryptText(message, key, operation);
                    return cipher;
                } else if (mode.equals("OFB")) {
                    byte[] temp = cryptText(iv, key, operation);
                    cipher = XORBytes(temp, message);
                    return cipher;
                }
            }
            int i = 0;
    ...
    public static void main(String[] args) {
        try {
            String mode = "CBC";
            Encryptor des = new Encryptor(mode);

            byte[] key = "12345678".getBytes();
            byte[] iv =  "12345678".getBytes();
            byte[] message = "kazancev".getBytes();

            byte[] cipher = des.crypt(message, key, iv, "encrypt");

            byte[] plainText = des.crypt(cipher, key, iv, "decrypt");
            ...
            // cipher1.init(Cipher.DECRYPT_MODE, key1, encryptionIv);
            // plainText = cipher1.doFinal(cipher);

            System.out.println("CBC crypt " + new String(cipher));
            System.out.println("CBC decrypt " + new String(plainText));

            des = new Encryptor("OFB");
            cipher = des.crypt(message, key, iv, "encrypt");
            plainText = des.crypt(cipher, key, iv, "encrypt");

            System.out.println("OFB crypt " + new String(cipher));
            System.out.println("OFB decrypt " + new String(plainText));
